...
public class ClientsBuilder {
  ...
  public static Clients buildClients(JsonObject conf) {
    ...
    JsonObject cs = conf.getJsonObject("clients");
    if (cs != null) {
      for (String name : cs.fieldNames()) {
        Client client = (Client) buildObjectTree(cs.getJsonObject(name));
        if (client == null) {
  ...
  private static Object buildObjectTree(JsonObject o) {
    ...
    try {
      ...
      JsonObject props = o.getJsonObject("props");
      if (props != null) {
        for (String field : props.fieldNames()) {
